...
void app_main(void)
{
    ...
    while (1) {
        ...
        memset(tx_buf_recv, 0, TRANSFER_SIZE); // Dummy TX data
        ...
        ESP_LOGI(TAG, "Received buffer; first byte: 0x%02X, 0x%02X 0x%02X 0x%02X ... 0x%02X 0x%02X ..", rx_buf_recv[0], rx_buf_recv[1], rx_buf_recv[2], rx_buf_recv[3], rx_buf_recv[TRANSFER_SIZE - 2], rx_buf_recv[TRANSFER_SIZE - 1]);

        // ----- Transaction 2: Echo the received buffer back to Master -----
        memcpy(tx_buf_resp, rx_buf_recv, TRANSFER_SIZE);
        memset(rx_buf_resp, 0, TRANSFER_SIZE);
        t_resp.length    = TRANSFER_SIZE * 8;
        t_resp.tx_buffer = tx_buf_resp;
        t_resp.rx_buffer = rx_buf_resp;

        ret = spi_slave_transmit(RCV_HOST, &t_resp, portMAX_DELAY);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Echo transaction failed: %s", esp_err_to_name(ret));
        } else {
            ESP_LOGI(TAG, "Echoed buffer back to master; first byte: 0x%02X", tx_buf_resp[0]);
        }
    }
}
