...
def fsm_image_transfer(sock, image_path, user_id):
    try:
    ...
    while state not in (State.COMPLETE, State.ERROR):
        if state == State.SEND_START_WRITE:
            filename = f"{user_id}_{image_path.split('/')[-1]}"
            pkt = create_start_packet(filename)
...
def fsm_file_read(sock, output_filename, user_id):
    if output_filename.lower().endswith((".jpg", ".jpeg", ".jpeg")):
        output_filename = output_filename.split(".")[0] + ".png"
        print(output_filename)

    filename = f"{user_id}_{output_filename.split('/')[-1]}"
    start_read_pkt = create_start_read_packet(filename)
    print("Sending filename: ", filename)
...
def fsm_compute(sock, filename):
    ...
    filename = f"{user_id}_{filename.split('/')[-1]}"
...
def main():
    if len(sys.argv) != 5:
        print("Usage: python transfer_image.py <port> <ESP32_IP> <file> <mode> <user_id>")
        print(
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        if mode == "write":
            fsm_image_transfer(sock, file_arg, user_id)
        elif mode == "read":
            fsm_file_read(sock, file_arg, user_id)
        elif mode == "compute":
            fsm_compute(sock, file_arg, user_id)
        else:
