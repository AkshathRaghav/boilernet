        ...
        switch (fsm) {
            ...
            case BLADE_FSM_PROCESS:
            {
              ...
              if (pkt == PACKET_TYPE_FUC)
              {
                ...
                // memset(tensor_arena, 0, TENSOR_ARENA_SZ);
                ...
                ESP_LOGI(COMM_TAG, "INPUT quant params → scale=%.6f, zero_pt=%d", scale_in, zp_in);


                t4 = esp_timer_get_time();

                int8_t* in_data_ptr = input->data.int8;
                ...
                for (int y = 0; y < h; y++) {
                  for (int x = 0; x < w; x++) {
                    ...
                    int rgb_idx  = (y*w + x)*3;
                    float rf = rgba_ps[rgba_idx + 0] / 255.f;
                    float gf = rgba_ps[rgba_idx + 1] / 255.f;
                    float bf = rgba_ps[rgba_idx + 2] / 255.f;
                    in_data_ptr[rgb_idx + 0] = quantize(rf*2.f - 1.f);
                    in_data_ptr[rgb_idx + 1] = quantize(gf*2.f - 1.f);
                    in_data_ptr[rgb_idx + 2] = quantize(bf*2.f - 1.f);
                  }
                }
                // Print first few quantized values for manual cross-check
                ESP_LOGI(COMM_TAG, "in_data[0..5] = %d, %d, %d, %d, %d, %d",
                ...
                ESP_LOGI(COMM_TAG, "OUTPUT quant params → scale=%.6f, zero_pt=%d", scale_out, zp_out);

                // 8) Extract raw & dequantized outputs
                int8_t* out_data_ptr = output->data.int8;
                ESP_LOGI(COMM_TAG, "raw out_data: %d, %d, %d, %d, %d",
                        out_data_ptr[0], out_data_ptr[1],
                        out_data_ptr[2], out_data_ptr[3],
                        out_data_ptr[4]);  // adjust if >5 classes


                float confidences[NUM_CLASSES];
                for (int i = 0; i < NUM_CLASSES; i++) {
                  confidences[i] = (out_data_ptr[i] - zp_out) * scale_out;
                  ESP_LOGI(COMM_TAG,
                          "class %2d: raw=%4d → conf=%.4f",
                          i, out_data_ptr[i], confidences[i]);
                }
                ...
                ESP_LOGI(COMM_TAG, "Best index=%d (%s)", best, model_labels[best]);
